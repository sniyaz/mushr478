...
class ParticleFilter:
    ...
    def __init__(self, motion_params=None, sensor_params=None, **kwargs):
        ...
        try:
            ...
            base_to_laser = self._tf_buffer.lookup_transform(
                self.tf_prefix + "laser_link", self.tf_prefix + "base_link", rospy.Time(0)
            )
        except tf2_ros.LookupException as e:  # Will occur if odom frame does not exist
            rospy.logerr(str(e))
            rospy.logerr("failed to find odom")

        base_off, base_rot = utils.transform_stamped_to_pq(base_to_laser)

        # This additional rotation is applied because in MuSHR V4, the lidar is flipped 180 degrees
        self.laser_base_rot = transformations.euler_from_quaternion(base_rot)
    ...
    def _publish_tf(self):
        ...
        while not rospy.is_shutdown():
            ...
            # This additional np.pi rotation is applied because in MuSHR V4, the lidar is flipped 180 degrees
            # This rotation should ideally be done through transforms published by the urdf, however, it did not seem worth the effort at the time
            pa[2] += self.laser_base_rot[2]
